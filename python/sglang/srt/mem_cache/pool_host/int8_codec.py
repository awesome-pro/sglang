...
Self-contained by design: this module imports nothing from the ``hiqcache``
standalone repo so that the SGLang fork stays shippable on its own. The two
implementations are kept honest by
``test/registered/unit/mem_cache/test_hicache_int8_pool_host_unit.py``, which
asserts this codec agrees bit for bit with the reference implementation.
